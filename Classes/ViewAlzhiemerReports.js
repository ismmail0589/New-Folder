...
const ViewAlzhiemerReports = (props) => {
  ...
  const getSpecificReport = async (itemIndex) => {
    try {
      const dataPatient = await database().ref(`Reports/Alzhiemer/${itemIndex}`).on('value', tempdata => {
        ...
        props.navigation.navigate('DetailedAlzhiemerReport')
      });
  ...
  const data = await database().ref('Reports/Alzhiemer')
    ...
    try {
      Alert.alert('Alert', `Are you sure to delete ${itemValue}`, [
        ...
        },
        {
          text: 'OK',
          onPress: async () => {
            await database().ref(`Reports/Alzhiemer/${itemIndex}`).remove();
          }
            ...
            return (
              ...
              <View style={styles.Report1}>
                ...
                <View style={styles.Report1Text}>
                  <Text style={{ fontSize: 16, fontWeight: 'bold', color: 'black' }}>Alzheimer Report</Text>
...
export default ViewAlzhiemerReports;
