...
export default function Check({navigation, route}) {
  ...
  const generateReportNumber = () => {
    const prefix = 'MSL'; // You can customize the prefix if needed
  ...
  useEffect(() => {
    ...
    if (prediction.index === 0) {
      setDescription('MS-Sagittal');
    } else if (prediction.index === 1) {
      setDescription('MS-Axial');
    } else if (prediction.index === 2) {
      setDescription('Control-Sagittal');
    } else if (prediction.index === 3) {
      setDescription('Control-Axial');
    } else {
      console.log('No data');
    }
    const now = new Date();
  ...
  const addReports = async () => {
    try {
      ...
      await database().ref(`Reports/MultiSclerosis/${index}`).set({
        Time: time,
  ...
  return (
    <View style={styles.report}>
      <ViewShot ref={ref} options={{ fileName: "MultiSclerosis Report", format: "jpg", quality: 0.9 }} style = {styles.report}>
      <View style = {styles.report}>
        <Text style={{
          fontSize: 25, color: 'black', fontWeight: 'bold', alignSelf: 'center', position: 'absolute', top:-25
        }}>
          Multisclerosis Report
        </Text>
