...
const ViewSclerosisReport = (props) => {
  ...
  const getSpecificReport = async (itemIndex) => {
    try {
      const dataPatient = await database().ref(`Reports/MultiSclerosis/${itemIndex}`).on('value', tempdata => {
        ...
        props.navigation.navigate('DetailedSclerosisReport')
      });
  ...
  const data = await database().ref('Reports/MultiSclerosis')
    ...
    try {
      Alert.alert('Alert', `Are you sure to delete ${itemValue}`, [
        ...
        },
        {
          text: 'OK',
          onPress: async () => {
            await database().ref(`Reports/MultiSclerosis/${itemIndex}`).remove();
          }
            ...
            return (
              ...
              <View style={styles.Report1}>
                ...
                <View style={styles.Report1Text}>
                  <Text style={{ fontSize: 16, fontWeight: 'bold', color: 'black' }}>MultiSclerosis Report</Text>
...
export default ViewSclerosisReport;
